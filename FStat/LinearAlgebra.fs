...
module DenseUnits = 
   ...
   type Vectoru<[<Measure>] 'u>  =
      ...
      //let defaultArg v o = function | Some(v) -> v | _ -> v
                         
   type Matrixu<[<Measure>] 'u>  =
      ...
      static member (*)(leftSide:Matrixu<'u>, rightSide:Matrixu<'v>) =
...
module DenseMatrixU =
    ...
    let inline diag (v: #DenseUnits.Vectoru<'u>) =
        let n = v.Count
        let A = new DenseUnits.Matrixu<'u>(n,n)
        for i=0 to n-1 do
            A.[i,i] <- v.Item(i)
        A
    ...
    let toto =    
      ...
      let a = diag(DenseUnits.Vectoru(meters))  //[|5.;1.;3.|]
      let b = diag(DenseUnits.Vectoru(seconds))  //[|5.;1.;3.|]
      let c = a * b 
      ()


      //member
      //this.Data = Array.init size (fun _ -> _value);
